...
public class RotateAndTranslateAnimation extends Animation {
    ...
    protected Interpolator mTranslationInterpolator = null;
    protected Interpolator mRotationInterpolator = null;
    ...
    protected void applyTransformation(float interpolatedTime, Transformation t) {
        ...
        float rotationTime = interpolatedTime;
        if (mRotationInterpolator != null) {
            rotationTime = mRotationInterpolator.getInterpolation(interpolatedTime);
        }
        final float degrees = mFromDegrees + ((mToDegrees - mFromDegrees) * rotationTime);
        ...
        float translationTime = interpolatedTime;
        if (mTranslationInterpolator != null) {
            translationTime = mTranslationInterpolator.getInterpolation(interpolatedTime);
        }
        float dx = mFromXDelta;
        float dy = mFromYDelta;
        if (mFromXDelta != mToXDelta) {
            dx = mFromXDelta + ((mToXDelta - mFromXDelta) * translationTime);
        }
        if (mFromYDelta != mToYDelta) {
            dy = mFromYDelta + ((mToYDelta - mFromYDelta) * translationTime);
        }
    ...
    public void setTranslationInterpolator(Interpolator i) {
        mTranslationInterpolator = i;
    }

    public void setRotationInterpolator(Interpolator i) {
        mRotationInterpolator = i;
    }
