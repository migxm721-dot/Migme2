...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView> extends PullToRefreshBase<T> implements
    ...
    protected void addRefreshableView(Context context, T refreshableView) {
        if(mContainedView != null) {
            int index = indexOfChild(mContainedView);
            removeViewAt(index);
            addView(refreshableView, index, new LinearLayout.LayoutParams(LayoutParams.FILL_PARENT, 0, 1.0f));
        } else {
            addView(refreshableView, new LinearLayout.LayoutParams(LayoutParams.FILL_PARENT, 0, 1.0f));
        }
        mContainedView = refreshableView;
	};
